...
export default function ViewerIntelligence() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-white">Viewer Intelligence</h1>
        <p className="text-sm mt-1" style={{ color: "#8888A8" }}>Audience segmentation, churn risk, and LTV scoring</p>
      </div>

      {/* KPI Row */}
      <div className="grid grid-cols-4 gap-4">
        <StatCard label="Total Active Users" value="12,400" delta="Last 30 days" deltaType="neutral" icon={<Users size={16} />} />
        <StatCard label="High Churn Risk" value="1,240" delta="10% of active base" deltaType="down" icon={<AlertTriangle size={16} />} accent="#E74C3C" />
        <StatCard label="Avg LTV" value="₹3,200" delta="+₹120 vs last month" deltaType="up" icon={<DollarSign size={16} />} accent="#2ECC71" />
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            {/* Genre */}
            <div>
              <label className="text-xs font-medium mb-2 block" style={{ color: "#8888A8" }}>Genre Affinity (multi-select)</label>
            ...
            {/* Language */}
            <div>
              <label className="text-xs font-medium mb-2 block" style={{ color: "#8888A8" }}>Primary Language</label>
              <div className="flex flex-wrap gap-1.5">
                {LANG_OPTIONS.map(l => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium mb-2 block" style={{ color: "#8888A8" }}>Device Type</label>
                <div className="relative">
                  ...
                  <ChevronDown size={12} className="absolute right-2 top-2.5" style={{ color: "#8888A8", pointerEvents: "none" }} />
                </div>
              </div>
              <div>
                <label className="text-xs font-medium mb-2 block" style={{ color: "#8888A8" }}>Geo Tier</label>
                <div className="relative">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium mb-2 block" style={{ color: "#8888A8" }}>Age Band</label>
                <div className="relative">
              ...
              <div>
                <label className="text-xs font-medium mb-2 flex justify-between" style={{ color: "#8888A8" }}>
            ...
            {builtSegment ? (
              <div className="space-y-4">
                <div className="p-4 rounded-xl" style={{ background: "rgba(10, 123, 140, 0.08)", border: "1px solid rgba(10, 123, 140, 0.2)" }}>
                  ...
                  <div className="text-sm font-semibold text-white mb-4">{builtSegment.label}</div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="text-center p-3 rounded-lg" style={{ background: "#1E1E35" }}>
                      <div className="text-2xl font-bold font-mono text-white">{builtSegment.size.toLocaleString("en-IN")}</div>
                      <div className="text-[10px] mt-1" style={{ color: "#8888A8" }}>Estimated users</div>
                    </div>
                    <div className="text-center p-3 rounded-lg" style={{ background: "#1E1E35" }}>
                      <div className="text-2xl font-bold font-mono" style={{ color: "#F0A500" }}>₹{builtSegment.cpm}</div>
                      <div className="text-[10px] mt-1" style={{ color: "#8888A8" }}>Avg CPM</div>
                    </div>
                    <div className="text-center p-3 rounded-lg" style={{ background: "#1E1E35" }}>
                      <div className="text-2xl font-bold font-mono" style={{ color: builtSegment.churnPct > 15 ? "#E74C3C" : "#2ECC71" }}>
                        {builtSegment.churnPct}%
                      </div>
                      <div className="text-[10px] mt-1" style={{ color: "#8888A8" }}>Churn risk</div>
                    </div>
                    <div className="text-center p-3 rounded-lg" style={{ background: "#1E1E35" }}>
                      <div className="text-2xl font-bold font-mono" style={{ color: "#2ECC71" }}>₹{builtSegment.ltv.toLocaleString("en-IN")}</div>
                      <div className="text-[10px] mt-1" style={{ color: "#8888A8" }}>Avg LTV</div>
                    </div>
                  </div>
                  <button className="w-full mt-3 py-2 rounded-lg text-xs font-medium"
                    style={{ background: "#1E1E35", color: "#0A7B8C", border: "1px solid rgba(10,123,140,0.3)" }}>
              ...
              <div className="flex flex-col items-center justify-center h-full text-center p-6">
                ...
                <div className="text-sm font-medium" style={{ color: "#8888A8" }}>Configure filters and click</div>
                <div className="text-sm font-medium" style={{ color: "#555580" }}>Build Segment to see results</div>
                <div className="mt-4 text-xs" style={{ color: "#555580" }}>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={churnDistribution} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              ...
              <XAxis dataKey="range" tick={{ fill: "#555580", fontSize: 9 }} tickLine={false} />
              ...
              <Bar dataKey="count" name="Users" radius={[3, 3, 0, 0]}
                ...
                {churnDistribution.map((entry, i) => (
                  <rect key={i} fill={entry.range.startsWith("0.7") || entry.range.startsWith("0.8") || entry.range.startsWith("0.9") ? "#E74C3C" :
                    entry.range.startsWith("0.5") || entry.range.startsWith("0.6") ? "#F39C12" : "#0A7B8C"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          ...
          <div className="mt-3 p-3 rounded-lg flex items-start gap-2"
            ...
            <div className="text-xs">
              <span style={{ color: "#E74C3C" }} className="font-medium">820 users at high risk (score &gt;0.70) </span>
              <span style={{ color: "#8888A8" }}>— top signals: 3+ session gaps, series abandonment at Ep.2, no new content in preferred genre</span>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={ltvData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="avgLTV" stroke="#2ECC71" strokeWidth={2} dot={false} name="Avg LTV" />
            </LineChart>
          </ResponsiveContainer>
          <div className="mt-3 p-2 rounded-lg text-xs text-center"
            style={{ background: "rgba(46, 204, 113, 0.08)", color: "#2ECC71" }}>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={watchTimeFunnel} layout="vertical" margin={{ top: 0, right: 10, left: 60, bottom: 0 }}>
              <XAxis type="number" tick={{ fill: "#555580", fontSize: 10 }} tickLine={false} axisLine={false} />
              <YAxis type="category" dataKey="stage" tick={{ fill: "#8888A8", fontSize: 10 }} tickLine={false} axisLine={false} />
              <Tooltip contentStyle={{ background: "#1A1A2E", border: "1px solid #2A2A45", borderRadius: 8 }}
                itemStyle={{ color: "#E8E8F0" }} />
              <Bar dataKey="metro" name="Metro" fill="#0A7B8C" radius={[0, 3, 3, 0]} />
              <Bar dataKey="diaspora" name="Diaspora" fill="#F0A500" radius={[0, 3, 3, 0]} />
              <Bar dataKey="t2" name="T2" fill="#2ECC71" radius={[0, 3, 3, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>
        ...
        <Card>
          ...
          <div className="space-y-3">
            {[demoUsers["CH-7733"], demoUsers["CH-2290"]].map(user => (
              <div key={user.id} className="p-3 rounded-lg" style={{ background: "#1E1E35", border: "1px solid #2A2A45" }}>
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-xs font-semibold text-white">{user.id} · {user.name}</div>
                      <div className="text-[10px]" style={{ color: "#8888A8" }}>{user.location}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold font-mono" style={{ color: user.churnRisk > 0.5 ? "#E74C3C" : "#F39C12" }}>
                      {user.churnRisk.toFixed(2)}
                    </div>
                    <div className="text-[10px]" style={{ color: "#8888A8" }}>churn risk</div>
                  </div>
                </div>
                <div className="space-y-1">
                  {user.profileDrift.slice(0, 2).map((d, i) => (
                    <div key={i} className="flex items-center gap-1.5 text-[10px]">
                      <span style={{ color: d.direction === "down" ? "#E74C3C" : d.direction === "up" ? "#2ECC71" : "#8888A8" }}>
